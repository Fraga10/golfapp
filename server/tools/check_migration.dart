...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final players = await DB.conn.query(
        "SELECT column_name FROM information_schema.columns WHERE table_name='game_players'");
    stderr
        .writeln('game_players columns: ${players.map((r) => r[0]).toList()}');
    final rounds = await DB.conn.query(
    ...
    final strokes = await DB.conn.query(
